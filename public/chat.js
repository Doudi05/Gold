...
document.addEventListener("DOMContentLoaded", function () {
  ...
  let btnGoldCreate = document.getElementById("btngoldCreate");
  let goldFrame = document.getElementById("frame");
  let goldFooter = document.querySelector("#gold > footer");
  ...
  btnGoldCreate.onclick = () => {
    sock.emit("gold", { action: "create" });
  };
  ...
  /**
   * Handler for different gold event
   */
  const goldHandler = Object.create(null);

  goldHandler.create = function (data) {
    createGoldLobby(data.game);
    speak(`Vous venez de créer une nouvelle partie !`);
  };

  goldHandler.invite = function (data) {
    ...
    createGoldInvitation(data.req.from, data.req.game);
  };

  goldHandler.data = function (data) {
    ...
    if (status === "pending" && gameData.players[pseudo].status === "ready") {
      ...
      if (gameData.board.some((c) => c !== null)) {
        ...
        if (gameData.nthRound > 1) {
          ...
          let board = document.querySelector(`#gold .game[data-gameid="${data.req.game}"] .board`);
          ...
          let gameFrame = document.querySelector(`#gold .game[data-gameid="${data.req.game}"]`);
  ...
  goldHandler.remove = function (data) {
    if (data.status === 1 || data.req.player === pseudo) {
      ...
      sock.emit("gold", { action: "data", game: data.req.game });
    }
  };

  goldHandler.join = function (data) {
    if (data.req.player === pseudo) {
      // If the user join a game -> create the lobby element
      deleteGameTab(data.req.game);
      createGoldLobby(data.req.game);
  ...
  goldHandler.launch = function (data) {
    sock.emit("gold", { action: "data", game: data.req.game });
  };

  goldHandler.play = function (data) {
    ...
    if (data.status === 1) {
      ...
      if (data.haveToNext === pseudo) {
        console.log("next");
        sock.emit("gold", { action: "next", game: data.req.game });
      }
  ...
  goldHandler.winner = function (data) {
   ...
   * @param {object} data
   */
  function handleGoldEvent(data) {
    // console.dir(data);

    if (!data || ((!data.req || !data.req.action) && !data.action)) {
      console.error("[gold] - Invalid data received from server ");
      return;
    }
    ...
    if (goldHandler[action]) {
      goldHandler[action](data);
    } else {
   ...
   */
  function quitGame() {
    let gameTab = document.querySelector("#gold input[name='gold-frame']:checked + .game");
    if (gameTab) {
      let id = gameTab.dataset.gameid;
      sock.emit("gold", { action: "remove", game: id, player: pseudo });
    }
  }

  /**
   * Update the player card in a gold game
  ...
  function updatePlayerCard(id, card) {
    let cardElt = document.querySelector(
      `#gold .game.play[data-gameid="${id}"] > .player[data-pseudo="${pseudo}"] .card `
    );
  ...
  function createGameTab(id, type) {
    ...
    goldFrame.appendChild(input);
    goldFrame.appendChild(div);
    goldFooter.appendChild(label);

    return div;
  }

  /**
   * Create a new gold lobby
   * @param {number} id The game id
   * @param {object[]} players The list of player in this game
   */
  function createGoldLobby(id, players) {
    ...
    let section = elt(
      ...
      createButton("Lancer la partie !", {}, () => {
        sock.emit("gold", { action: "launch", game: id });
      }),
  ...
  function createCard(type, color) {
    return elt(
      ...
      elt(
        "div",
        { class: "back" },
        elt("p", { class: "gold-logo" }, ..."gold".split("").map((l) => elt("span", {}, l)))
      )
    );
  }
  ...
  function createLaunchedGame(id, gameData) {
    ...
    board.onclick = (e) => {
      ...
      reminders.remove(id);
      sock.emit("gold", { action: "play", game: id, index, player: pseudo });
    };
  ...
  function deleteGameTab(id) {
    reminders.remove(id);
    let div = document.querySelector(`#gold .game[data-gameid="${id}"]`);
    if (!div) {
      return;
    }
    div.remove();
    let radio = document.querySelector(`#gold input[id="radio-game-${id}"]`);
    // log(radio);
    radio.remove();
    let label = document.querySelector(`#gold footer label[for="radio-game-${id}"]`);
    // log(label);
    label.remove();
    home_radio.checked = true;
  }
  ...
  /**
   * Create the gold invitation screen
   * @param {string} from The player who invites
   * @param {number} id The game ID
   */
  function createGoldInvitation(from, id) {
    let div = createGameTab(id, "join");

    let section = elt(
      "section",
      {},
      elt("h2", {}, `${from} vous invite dans une partie de gold !`),
      (() => {
        let btn = createButton("Rejoindre !");
        btn.onclick = () => {
          sock.emit("gold", { action: "join", player: pseudo, game: id });
        };
        return btn;
      })()
    );
  ...
  function updatePlayersList(id, players) {
    ...
    const aside = document.querySelector(`#gold .game.launch[data-gameid='${id}'] aside`);
    if (!aside) {
      return;
    }
  ...
  /**
   * Update the players status during a gold game (check if a player left the game)
  ...
  function updatePlayersStatus(game, data) {
    for (let p in data.players) {
      if (data.players[p].status === "left") {
        let player = document.querySelector(`#gold .game[data-gameid="${game}"] .player[data-pseudo="${p}"]`);
        if (player) {
  ...
  function updateList(newList) {
    ...
    for (let game of document.querySelectorAll(".game:not(.join)")) {
      sock.emit("gold", { action: "data", game: game.dataset.gameid });
    }
  }
  ...
  async function playCardAnimation(game, card, index, player) {
    // log(player);
    let cardElt = document.querySelector(
      `#gold .game.play[data-gameid="${game}"] > .player[data-pseudo="${player}"] .card `
    );
    ...
    let target = document.querySelector(
      `#gold .game.play[data-gameid="${game}"] > .board:not(.past) .card:nth-child(${index + 1})`
    );
